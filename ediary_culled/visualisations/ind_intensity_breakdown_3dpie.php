<?php
	...
	$res = mysql_query('select c.intensity as intensity,sum(tr1.duration) as total_mins,Sum(tr1.duration * c.mets) as total_mets,Sum(tr1.duration * c.mets) / (select Sum(tr1.duration * c.mets) from training_records1 tr1, compcodes c
where tr1.class ="'.$class.'" and tr1.student_id='.$id.' and daydate<="'.$date.'" and c.compcode=tr1.compcode) * 100 as prop
from training_records1 tr1, compcodes c
where tr1.class ="'.$class.'" and tr1.student_id='.$id.' and daydate<="'.$date.'" and c.compcode=tr1.compcode
group by Intensity;
');
	
	while($row = mysql_fetch_array($res)) {
		$data[0][] = $row['intensity'];
		$data[1][] = $row['prop'];
	}
