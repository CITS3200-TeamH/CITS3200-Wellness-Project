<?php
	...
	$sql2= "select tr1.daydate as date,tr1.student_id as id, compcodes.type as type, sum(tr1.duration) as duration, sum(tr1.duration*compcodes.mets) as mets,compcodes.cardio as cardio,compcodes.muscle as muscle,compcodes.flex as flex,compcodes.body as body, compcodes.heading as heading ,compcodes.description as description,compcodes.intensity as intensity
	FROM training_records1 tr1,compcodes
	where compcodes.compcode = tr1.compcode  and tr1.class = '$name' and tr1.student_id='$id' and tr1.daydate<='$date' group by daydate
	order by daydate";
	...
	//CSV HEADINGS
	$csv .= 'Date,ID,Total Duration,Total METs';
	$j = 0;
	if($res4) {
	...
	for($i=0;$i<$ndays;$i++) {
		...
		if($row['date'] == $daydate) {
			...
			while($row['date'] == $daydate) {
				$csv .= $daydate.','.$id.','.$row['duration'].','.$row['mets'].',';
